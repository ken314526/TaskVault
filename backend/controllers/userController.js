...
exports.deleteUser = async (req, res) => {
  try {
    ...
    // Cascading: Unassign user from all tasks
    await Task.updateMany({ assignedTo: user._id }, { assignedTo: null });

    // Cascading: Remove user from all project members arrays
    await Project.updateMany(
      { "members.user": user._id }, 
      { $pull: { members: { user: user._id } } }
    );

    // Cascading Cleanup: Delete projects that now have 0 members
...
exports.seedAdmin = async (req, res) => {
  ...
  try {
    const adminEmail = process.env.SUPERADMIN_EMAIL || 'superadmin@test.com';
